...
每站点可以绑定一套策略：单域名并发数、最小/最大间隔、每分钟请求上限、每日上限、连接/读取超时、重试次数和失败熔断阈值。默认值是并发 1、间隔 3-8 秒、遇到 429 遵守 `Retry-After`。`Retry-After` 字段保留用于兼容旧数据，但后端强制为开启，不能通过管理端或接口关闭。
...
1. 创建站点时展示 robots.txt、黑名单、地址和连通性风险；授权关系由管理员根据站点外部的合同、版权方、合作方或其他合法材料自行确认并记录，不由系统臆测或替管理员作出授权结论。
...
管理端可以按站点实际公开规则配置并发、请求间隔、分钟/日上限、超时、有限重试次数和熔断阈值。系统将风险检测与人工确认分开：管理员可以在风险信息基础上填写外部授权来源并确认是否允许采集，然后通过“采集许可”开关自主控制该站点是否接收新任务；新站点默认停用只是初始状态，不是系统对授权关系的否定。采集许可只控制是否运行，不代表可以关闭安全门禁。编辑站点的备注、策略等普通字段不会自动撤销既有许可，只有访问地址变化才要求重新确认。

以下门禁不是业务偏好，不能做成可关闭的采集开关：

- 黑名单命中后禁止请求。
- robots.txt 明确禁止或无法检查时禁止继续。
- 仅允许公网 HTTP(S) 地址，禁止 SSRF、内网、环回和云元数据地址。
- 仅允许同一已审核主机，禁止跨主机跳转。
- 401/403 自动暂停并要求人工复核。
- 429 必须遵循 `Retry-After`，否则按策略退避并暂停/失败。
- 采集结果必须经过人工审核，不能自动发布。
- 禁止代理轮换、验证码/付费墙绕过、指纹伪装和隐藏身份请求。

## 7. 管理端页面

第一阶段页面入口统一放在“阅读器管理 > 书源采集中心”：

- 书源站点：站点 CRUD、风险信息与外部授权来源确认、独立采集许可启停、策略绑定和连通性测试。
...
{
  "approved": true,
  "authorizationNote": "版权方合作合同 CN-2026-001，授权范围包含本系统公开阅读服务，有效期至 2027-09-01"
}
```

其中 `approved` 是管理员基于外部授权材料作出的业务选择，不是平台对目标站点授权关系的验证结果；`authorizationNote` 在允许采集时必填并进入审计记录。保存后，管理员仍需单独打开“采集许可”开关，风险门禁不因该确认而关闭。
...
当前代码进度：站点、策略、规则、任务、运行记录的持久化模型、管理端 CRUD、风险信息与授权来源确认、独立采集许可、站点/规则状态流转、任务启动暂停恢复取消和运行记录查询已完成；Worker HTTP 协议、Redis 分布式租约/动态限流、章节快照入库、哈希校验、错误记录和待审核边界已完成。Python/Go 参考 Worker 已提供，正式部署仍需配置环境变量，并由管理员根据外部授权材料自主确认站点许可。
...
- 禁止未完成授权来源确认且未启用采集许可的站点进入 RUNNING。
