...
本仓库是阅读器后端和采集中心的核心服务层，PC 管理端负责运营操作，H5 负责读者阅读。下面的页面截图入口分别对应两个前端仓库；本仓库的架构图和流程图用于说明服务端如何承接这些页面请求。

### 服务端承接的阅读链路

![阅读器页面与 API 链路](demo/reader-api-flow.svg)

H5 从首页、书城或搜索进入作品详情，再通过目录分页读取章节正文；服务端统一返回作品分类、作者、连载状态、横竖版封面和规范化正文，并保存书架、书签与阅读进度。

### 服务端承接的采集链路

![采集中心主任务与备用书源链路](demo/source-collection-flow.svg)

采集中心以站点授权为边界，依次经过策略限流、规则解析、主任务、Worker 租约、章节快照、审核和发布。主站点遇到额度、403、熔断或解析异常时，只会从已登记且已授权的站点生成备用子任务，继续补齐缺失章节。

> 两张图是文档示意图，详细交互时序、状态和数据表说明见 [`docs/reader-platform-architecture.md`](docs/reader-platform-architecture.md)。
